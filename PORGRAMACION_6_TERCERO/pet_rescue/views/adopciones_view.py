...
class AdopcionesView:
    ...
    def guardar_adopcion(self, e):
        if not all([self.nombre_field.value, self.apellido_field.value, 
            ...
            self.app.show_snackbar("❌ Los campos marcados con * son obligatorios", "#F44336")
            return
        ...
        try:
            ...
            self.app.show_snackbar(
                f"✅ ADOPCIÓN REALIZADA CON ÉXITO! {nombre_animal} tiene un nuevo hogar 🏠", 
                "#4CAF50"
            )
            ...
            self.app.show_snackbar(f"❌ Error al registrar adopción: {str(ex)}", "#F44336")
